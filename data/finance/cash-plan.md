# CASH PLAN - from 1 Oct 2026. Written 26 Sep by Finance. All figures "around"; lines marked ? are assumptions Calum corrects.

## The system: three pots, filled on payday, in this order
1. FIXED - leaves whether you like it or not. One number a month. Sits in Mashreq (AED) and BoS (GBP) before anything else moves.
2. LIVING - everything you choose. One number a week. Spent from ONE card (Wise or ADIB debit - never a credit card).
3. DEBT - whatever is left after 1 and 2. Goes to the dearest debt first. Never funds 1 or 2.
Budgeting = fill pot 1, cap pot 2, and pot 3 is what's left. Nothing else.

## October v2 - Calum's numbers (26 Sep), with what the statements say
| Line | Calum | AED | Statements Apr-Sep | Verdict |
|---|---|---|---|---|
| UK card minimums | GBP1,500 | 7,400 | GBP1,400-1,600/mo | right; falls as cards clear |
| Kevin | GBP1,000 | 4,900 | GBP2,000 in Aug and Sep | new committed line |
| Lewis | 10,000 | 10,000 | 44,775 repaid YTD, lumpy | agreed number |
| Tal - cards, living, bills | GBP5,000 | 24,600 | ~45,000/mo sent | a 45% cut - real if Tal agrees it |
| Rent - separate account | 15,000 | 15,000 | not visible in any statement | pot made physical - good |
| Tara - separate account | 31,000 | 31,000 | own instalments not yet separated from co-investors' | good; send the schedule |
| ADIB card: pay 70K, spend 25K | 70,000 | 70,000 | 279K paid to it YTD | clears ~64.6K + living 25K; card must be at zero every payday or it is 40% again |
| Murabaha | - | 2,409 | monthly | forgotten |
| Editors | - | 6,000 | Apr-Sep avg | forgotten - or zero if the brand pauses |
| Phone, subs, software | - | 2,500 | recurring | forgotten |
| Coaching GBP460 | - | 2,260 | monthly since Mar | forgotten - keep or cancel |
| Meta ads | - | 0-10,000 | Jun 10.6K, Jul-Sep 0 | your call |
| **October cash out** | **163K** | **~176-186K** | | |
Of the 70K to ADIB, 45K is debt paydown, so October's true running cost is ~130-140K/mo once the card is at zero.

## October v3 - payday now expected ~Đ180K, not 314K (Calum 26 Sep: only a few deals paying; the rest slips)
Fixed: UK cards 7.4K (send GBP2,000 to BoS: minimums 1,500 + Agria/pets/Apple/Giffgaff ~190 + OD interest + margin) · Kevin 4.9K · Lewis 10K · Tal 24.6K · rent 15K · Tara 31K · Murabaha 2.4K (Calum to confirm what it is - ADIB instalment 2,408.70 seen Jan/Feb/Apr) · editors 6K (last month) · subs/software 2.5K · coaching 2.3K · ads 0 = **~106K** (~100K from November: no editors)
Living: 25K on the ADIB card, paid to zero on payday.
ADIB card: 70K in (clears ~64.6K + the 25K living) -> 
Total October: ~176K. Payday ~180K. Spare: ~4K. November: NOTHING unless White & Co pay the slipped September deals + October deals post-exit (~296K owed, none confirmed in writing).
Lever if the October payout is not in writing by 30 Sep: pay ADIB 40K not 70K (stays under limit, covers living), hold 30K for November rent/Tara. Cost ~Đ1K/mo interest; buys November. Tara 31K: if it is an accrual (not due in Oct) it is the second lever.

## The runway on these numbers (v2, superseded by v3)
30 Sep Đ314K -> October ~180K -> ~134K left. November needs ~120K (no ADIB clearing to repeat). So November works ONLY if White & Co's October payout (~162K) lands post-exit. Get it in writing before 1 Oct. Jake's first real pay ~Dec.

## Pot 1 - FIXED, October v1 (superseded by the table above; kept for reference)
| Line | AED/mo | Basis |
|---|---|---|
| Tal + home + baby | 45,000 | Apr-Sep average of money sent to Tal |
| UK card DDs + interest (9 cards) | 8,000 | ~GBP1,600/mo at today's balances - falls as they're paid |
| ADIB Murabaha | 2,409 | monthly instalment |
| Editors (Contentus, Abegail, Ali Vids) | 6,000 | Apr-Sep average |
| Meta ads | 10,000 ? | your call - Jun ran 10.6K, Jul-Sep zero |
| Phone, subs, software/AI | 2,500 | Wise + Amex recurring |
| Coaching (SP Shop Plymouth / A. Butler) | 2,260 ? | GBP460/mo - confirm it's still wanted |
| Methil property | 0 | rent covers mortgage + OD if the rent is left in the account |
| Lewis Page repayment | 10,000 ? | GBP205K-equivalent still owed - agree a number |
| Rent (Dubai) | ? | not visible in any statement - amount + next cheque date needed |
| Modon Hudayriyat / Tara instalments | ? | dated - schedule needed |
| **Fixed, before rent and instalments** | **~86,000** | |

## Pot 2 - LIVING, October
Đ25,000 for the month = Đ6,000 a week. Food, deliveries, fuel, eating out, nights out, gifts, anything.
One card. Weekly check on Sunday: spent vs 6,000. Over = the next week is shorter. No top-ups from a credit card, ever.
(Apr-Sep this ran Đ25-40K/mo from the banks plus whatever the ADIB card carried - so 25K is a real cut, not a wish.)

## Pot 3 - DEBT, October
What is left. Order: ADIB card (40%) -> Aqua (42% cash) -> Tesco (27%) -> Virgin (24%) -> Santander (22%) -> M&S -> MBNA -> HSBC -> Barclaycard (0% BT tranche last).

## Payday 30 Sep - Đ314,100 expected (reconcile vs payslip). Move it the same day:
| Step | AED | Where |
|---|---|---|
| 1. October FIXED | 86,000 | stays in Mashreq; GBP2,000 to BoS for the UK DDs (BoS must not touch -GBP3K again) |
| 2. October LIVING | 25,000 | to the living card, once |
| 3. ADIB card cleared | 64,600 | in full - it is 40% |
| 4. November FIXED held | 86,000 | untouched in Mashreq - because White & Co's October payout is NOT confirmed in writing |
| 5. Remainder | ~52,000 | Lewis + Aqua, or held until 4 is released by the October payout |
Rent and instalments come out of steps 1/4 once the dates are known.

## Rules (dated)
- 26 Sep: No cash advances - a credit card never funds Wise or an ATM.
- 26 Sep: Living spend from one card only, Đ6K a week, checked Sunday.
- 26 Sep: Payday order is Fixed -> Living -> Debt. Debt never funds the first two.
- 26 Sep: Money crosses UAE-UK on Wise only.
- 26 Sep: Every cash withdrawal gets a why the same day.

## What I check
Sunday: living vs 6K. Month end: fixed lines vs actual, fees (target < Đ2K), kept/broken on each rule, runway recomputed.
